...
import { parseGeminiError, formatErrorForUser, validateApiKey, createWebGeminiError } from '../platform/errors.js';
...
describe('Web Error Handling', () => {
  describe('parseGeminiError', () => {
    it('should parse 401 unauthorized errors', () => {
      ...
      expect(result.message).toBe('Invalid API key. Please check your Gemini API key and try again.');
    ...
    it('should parse 500 server errors', () => {
      ...
      expect(result.message).toBe('Gemini API server error. Please try again in a moment.');
      expect(result.retryable).toBe(true);
    ...
    it('should parse JSON error responses', () => {
      const error = new Error('API error: {"error":{"code":400,"message":"Invalid request"}}');
