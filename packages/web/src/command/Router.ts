...
export class CommandRouter {
      ...
      if (stat.isDirectory()) {
        ...
        for (const file of filteredFiles.slice(0, 50)) {
          ...
          try {
            ...
            if (fileStat.isFile() && fileStat.size < 100000) {
              // Limit file size to 100KB
              const fileContent = (await opfsAdapter.readFile(filePath, {
                encoding: 'utf8',
              })) as string;
              content += `--- ${file} ---\n${fileContent}\n\n`;
              filePaths.push(file);
  ...
  private async handleGitCommand(
    ...
    if (args.length === 0) {
      return {
        type: 'error',
        content: 'Git command missing. Usage: !git <command>',
      };
    }
    ...
    try {
      ...
      switch (gitCmd) {
        ...
        case 'log': {
          ...
          const commits = await gitService.log(context.workingDirectory, {
            depth,
          });
        ...
        case 'branch':
          if (gitArgs.length > 0) {
            ...
            const checkout =
              gitArgs.includes('-b') || gitArgs.includes('--checkout');
            await gitService.branch(
            ...
            return {
              type: 'message',
              content: `${checkout ? 'Created and checked out' : 'Created'} branch '${branchName}'`,
            };
          } else {
            // List branches
        ...
        case 'checkout':
          ...
          return {
            type: 'message',
            content: `Switched to branch '${gitArgs[0]}'`,
          };
        ...
        case 'add': {
          ...
          if (filepath === '.') {
            // Add all files
            const statusFiles = await gitService.status(
              context.workingDirectory,
            );
            const untracked = statusFiles.filter(
              (s) => s.status === 'untracked' || s.status === 'modified',
            );
            ...
            return {
              type: 'message',
              content: `Added ${untracked.length} files to staging area`,
            };
          } else {
            await gitService.add(context.workingDirectory, filepath);
            return {
              type: 'message',
              content: `Added '${filepath}' to staging area`,
            };
          }
        }
        ...
        case 'commit': {
          ...
          if (!message) {
            return {
              type: 'error',
              content:
                'Usage: !git commit "message" or !git commit -m "message"',
            };
          }
          const commitId = await gitService.commit(
            context.workingDirectory,
            message,
          );
          return {
            type: 'message',
            content: `Created commit ${commitId.slice(0, 7)}: ${message}`,
          };
        }
        ...
        case 'clone': {
          if (gitArgs.length === 0) {
            return {
              type: 'error',
              content: 'Usage: !git clone <url> [directory]',
            };
          }
          const url = gitArgs[0];
          const targetDir =
          ...
          return {
            type: 'message',
            content: `Successfully cloned '${url}' to '${targetDir}'`,
          };
        }
        ...
        case 'pull':
          ...
          return {
            type: 'message',
            content: 'Successfully pulled changes from remote',
          };
        ...
        case 'push':
          ...
          return {
            type: 'message',
            content: 'Successfully pushed changes to remote',
          };
        ...
        case 'init':
          if (gitArgs.length > 0) {
            ...
            return {
              type: 'message',
              content: `Initialized empty Git repository in ${targetDir}`,
            };
          } else {
        ...
        default:
          return {
            type: 'error',
            content: `Git command '${gitCmd}' not yet implemented`,
          };
      }
    } catch (error) {
    ...
    try {
      if (!geminiService.isConfigured()) {
        return {
          type: 'error',
          content:
            'Gemini API key not configured. Use /config api-key <your-key> to set it up.',
        };
      }
...
`;
      ...
      return {
        type: 'message',
        content:
          'Created GEMINI.md. You can now ask me to analyze your project and populate it with relevant information.',
      };
    } catch (error) {
  ...
  private async handleTheme(
    ...
    return {
      type: 'message',
      content:
        'Theme selection dialog not yet implemented. Currently using dark theme.',
    };
  }
...
`;
  ...
  private async handlePwd(
    _args: string,
    context: CommandContext,
  ): Promise<CommandResult> {
    return {
      type: 'message',
      content: `Current directory: ${context.workingDirectory}`,
    };
  }
    ...
    switch (subcommand) {
      case 'api-key':
        ...
        try {
          ...
          return {
            type: 'message',
            content:
              'Gemini API key configured successfully! You can now send prompts.',
          };
        } catch (error) {
          return {
            type: 'error',
      ...
      case 'git-token':
        ...
        try {
          ...
          let settings = {};
          try {
            const settingsData = (await opfsAdapter.readFile(settingsPath, {
              encoding: 'utf8',
            })) as string;
          ...
          return {
            type: 'message',
            content:
              'GitHub token configured successfully! You can now push/pull/clone private repositories.',
          };
        } catch (error) {
          return {
            type: 'error',
