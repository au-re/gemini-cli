 ...
 *
 * This script analyzes a folder and generates a comprehensive markdown file
 * containing all relevant content for LLM understanding.
 *
 * Usage: bun scripts/generate-context.ts <folder-path> [output-file]
 *
 * Features:
 * - Analyzes folder structure
 * - Extracts content from relevant files
 * - Filters out binary files and common ignore patterns
 * - Provides file metadata and structure overview
 * - Respects common ignore patterns (.gitignore-like)
 */
...
// File extensions that are typically relevant for LLM context
...
// Maximum total content size (10MB)
...
function isRelevantFile(
  ...
  // Always include certain configuration files (removed the ones we want to skip)
  const configFiles = [
    'README.md',
    'CHANGELOG.md',
    'LICENSE',
  ...
  // Include files without extension if they're small and might be scripts
  if (!extension && size < 10000) {
    return true;
  }
...
async function analyzeFile(
  ...
  if (info.isDirectory) {
    ...
    if (!info.isRelevant) {
      info.reason = `Skipped directory: ${fileName}`;
    }
    return info;
  }

  if (shouldSkipFile(filePath, fileName, stats.size)) {
...
function generateDirectoryTree(files: FileInfo[], rootPath: string): string {
  ...
  for (const file of relevantFiles) {
    const dir = file.isDirectory
      ? file.relativePath
      : relative(rootPath, join(file.path, '..'));
    if (!dirs.has(dir)) {
      dirs.set(dir, []);
    }
    if (!file.isDirectory) {
  ...
  for (const dir of sortedDirs) {
    ...
    for (const file of dirFiles.sort((a, b) =>
...
function generateFileContent(files: FileInfo[]): string {
  const relevantFiles = files.filter(
    (f) => f.isRelevant && !f.isDirectory && f.content,
  );
...
async function main() {
  ...
  if (args.length === 0) {
    console.error('Usage: bun generate-context.ts <folder-path> [output-file]');
    process.exit(1);
  }
  ...
  console.log(
    `Relevant files: ${files.filter((f) => f.isRelevant && !f.isDirectory).length}`,
  );

  const markdown = [
    generateDirectoryTree(files, folderPath),
  ...
  console.log(
    `🔢 Estimated tokens: ${estimatedTokens.toLocaleString()} (~${Math.round(estimatedTokens / 1000)}k)`,
  );

  // Provide context about token limits
  if (estimatedTokens > 128000) {
    console.log(
      `⚠️  Warning: File exceeds typical LLM context limits (128k tokens)`,
    );
  } else if (estimatedTokens > 32000) {
    console.log(
      `⚠️  Note: File is large, may approach some LLM context limits`,
    );
  } else {
